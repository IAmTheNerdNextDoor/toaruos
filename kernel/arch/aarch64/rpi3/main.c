/**
 * @file  kernel/arch/aarch64/rpi3/main.c
 * @brief Boot stub for Raspberry Pi 3.
 *
 * This is about the same as K. Lange's RPi400 code but... y'know... RPi3?
 *
 * @copyright
    ...
    if (mbox_call(8) && mbox[fb_bpp] == 32 && mbox[fb_pointer] != 0) {
        ...
        for (unsigned int y = 0; y < lfb_resolution_y; ++y) {
            for (unsigned int x = 0; x < lfb_resolution_x; ++x) {
                *(volatile uint32_t *)((uintptr_t)lfb_vid_memory + y * lfb_resolution_s + x * 4) = 0x3ea3f0; // Light blue
            }
        }
...
void kmain(uint32_t dtb_address, uint32_t base_addr) {
    ...
    printf("rpi3 bootstub, kernel base address is %#x, dtb is at %#x\n", base_addr, dtb_address);
